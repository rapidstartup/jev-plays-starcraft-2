# Run LD-DG: preflight -> fresh controller -> jev-launch-traynor. Refuses SC2 boot on red.
...
& (Join-Path $root 'scripts\opencode\preflight-LD-DG.ps1')
if ($LASTEXITCODE -ne 0) { Write-Output 'RUN_ABORT LD-DG preflight red; SC2 not booted'; exit 1 }
