# Run LD-LJ: preflight -> fresh controller -> jev-launch-traynor. Refuses SC2 boot on red.
...
& (Join-Path $root 'scripts\opencode\preflight-LD-LJ.ps1')
if ($LASTEXITCODE -ne 0) { Write-Output 'RUN_ABORT LD-LJ preflight red; SC2 not booted'; exit 1 }
